...
# ----------------------------------------------------------------------
# ADMIN / DEBUG PŘEPÍNAČ
# ----------------------------------------------------------------------
# Když je ADMIN_CONTROL nastaveno na True:
#   - postava nemůže zemřít v BĚŽNÝCH ročních soubojích (run_yearly_fight) -
#     prohra pořád proběhne (a sníží fight_bonus), ale nikdy nezabije.
#   - postava nemůže dostat/přijmout nabídku vstupu do Akatsuki.
# Ostatní zdroje smrti (souboje s Akatsuki jako jinchūriki, lov Bijuu jako
# člen Akatsuki, dostižení jako nukenin, finální souboj s Jūbi) tímto
# přepínačem OVLIVNĚNY NEJSOU - jde čistě o "normální souboje" a Akatsuki
# nábor, přesně jak název napovídá.
ADMIN_CONTROL = False

# ----------------------------------------------------------------------
# ZÁKLADNÍ NASTAVENÍ
# ----------------------------------------------------------------------
# FULLSCREEN REŽIM - Automaticky se uzpůsobí rozlišení monitoru
...
# ----------------------------------------------------------------------
# LAYOUT HELPERS - vše se počítá z WIDTH/HEIGHT, takže je UI vždy
# vycentrované a přizpůsobené aktuálnímu rozlišení obrazovky.
# ----------------------------------------------------------------------
TOP_BAR_H = 100          # výška horního pruhu s titulkem
BOTTOM_MARGIN = 30       # rezerva od spodního okraje obrazovky
CONTENT_TOP = TOP_BAR_H + 15


def avail_height():
    """Kolik svislého místa zbývá pod top-barem a nad spodním okrajem."""
    return max(200, HEIGHT - TOP_BAR_H - BOTTOM_MARGIN)


def centered_top(block_height, top=CONTENT_TOP, bottom_margin=BOTTOM_MARGIN):
    """Vrátí Y souřadnici bloku o výšce block_height tak, aby byl
    svisle vycentrovaný v prostoru pod top-barem. Nikdy nejde nad `top`."""
...
def generic_layout(panel_height=250, button_h=55, gap=28, extra_top=0, back_h=50):
    """Sdílené rozvržení pro obrazovky s jedním panelem a tlačítky pod ním
    (VYTOČIT / ZPĚT DO MENU...). Používá se jak při kreslení, tak při
    stavbě tlačítek, aby byly vždy zarovnané a nikdy se nepřekrývaly.
    `extra_top` posune blok níž, pokud je nad ním např. panel s hinty.
    Výška panelu se navíc automaticky zmenší, pokud by se celý blok
    nevešel na výšku obrazovky (malé/nízké rozlišení)."""
...
def warning_layout(button_h=60):
    """Rozvržení pro obrazovky s varovným textem + jedním tlačítkem
    (např. 'nejdřív vytoč X'), vycentrované na výšku obrazovky."""
...
def summary_layout():
    """Sdílené rozvržení pro obrazovku KARTA POSTAVY (summary): výška
    panelu se přizpůsobí výšce obrazovky tak, aby vždy zbylo místo pro
    obě řady tlačítek dole a nic se nepřekrývalo."""
...
def fit_font(text, max_width, fonts):
    """Vrátí (font, text) - první font ze seznamu `fonts` (seřazený od
    největšího), do kterého se `text` vejde na šířku max_width. Pokud se
    nevejde ani do nejmenšího, text se zkrátí a doplní '…'."""
    for f in fonts:
...
BG_DARK   = (12, 12, 20)       # Ještě tmavší background
BG_PANEL  = (32, 32, 48)       # Lehce světlejší panely
BG_PANEL2 = (45, 45, 65)       # Alternativní panel barva
BG_PANEL3 = (20, 20, 32)       # Tmavší panel pro kontrast
ACCENT    = (255, 140, 0)      # Oranžový accent
ACCENT_HL = (255, 170, 60)     # Zvýrazněný accent
ACCENT_DARK = (200, 110, 0)    # Tmavší accent
BLUE      = (100, 180, 255)    # Jasnější modrá
BLUE_DARK = (70, 140, 200)     # Tmavší modrá
GOLD      = (255, 220, 80)     # Jasnější zlato
GOLD_DARK = (200, 170, 40)     # Tmavší zlato
GREEN     = (120, 230, 160)    # Jasnější zelená
RED       = (255, 100, 100)    # Jasnější červená
PURPLE    = (190, 130, 255)    # Jasnější fialová
PURPLE_DARK = (140, 80, 200)   # Tmavší fialová
WHITE     = (245, 245, 250)    # Čistě bílá
GRAY      = (160, 160, 180)    # Jasnější šedá
DARKGRAY  = (90, 90, 110)      # Tmavší šedá
SHADOW    = (0, 0, 0, 80)      # Pro shadow efekty
...
# ----------------------------------------------------------------------
# VIZUÁLNÍ HELPERY - gradienty, glow, lesk. Používá se v celém UI, aby
# panely a tlačítka nevypadaly jako ploché barevné obdélníky, ale měly
# trochu "hloubky" (jemný svislý gradient, měkký vnější glow v barvě
# kategorie, tenký lesklý proužek nahoře).
# ----------------------------------------------------------------------
...
def draw_gradient_rect(surf, rect, top_color, bottom_color, border_radius=0):
    """Vykreslí obdélník (volitelně se zaoblenými rohy) vyplněný svislým
    gradientem mezi top_color a bottom_color."""
...
def draw_glow_rect(surf, rect, color, border_radius=20, layers=5, max_alpha=55, spread=3):
    """Vykreslí měkkou, poloprůhlednou 'záři' v barvě `color` kolem obdélníku
    - použije se PŘED vykreslením samotného panelu/tlačítka, aby vypadalo,
    že jemně svítí do okolí."""
    rect = pygame.Rect(rect)
    pad = spread * layers + 2
    ...
    for i in range(layers, 0, -1):
        ...
        r.center = (cx, cy)
        pygame.draw.rect(glow, (*color[:3], alpha), r, border_radius=border_radius + spread * i)
...
def draw_soft_shadow(surf, rect, border_radius=18, offset=6, layers=3, max_alpha=110):
    """Měkčí varianta stínu než jeden plný černý obdélník - několik
    poloprůhledných vrstev s klesající alfou dává dojem jemného rozostření."""
    rect = pygame.Rect(rect)
    ...
    for i in range(layers, 0, -1):
        ...
        h = rect.height + i * 3
        r = pygame.Rect(0, 0, w, h)
        r.center = (cx, cy + offset)
        pygame.draw.rect(shadow, (0, 0, 0, alpha // layers), r, border_radius=border_radius + i)
...
def draw_panel(surf, rect, base_color, border_color, border_radius=20, border_width=3,
    ...
    """Standardní 'top' vzhled panelu použitý napříč hrou: měkký stín,
    volitelná barevná záře, svislý gradient výplně, zvýrazněný lesklý
    proužek nahoře a barevný okraj."""
    rect = pygame.Rect(rect)
    draw_soft_shadow(surf, rect, border_radius=border_radius)
    if glow:
        draw_glow_rect(surf, rect, border_color, border_radius=border_radius, layers=4, max_alpha=40)
    top_shade = tuple(min(255, c + 14) for c in base_color)
    bottom_shade = tuple(max(0, c - 10) for c in base_color)
    draw_gradient_rect(surf, rect, top_shade, bottom_shade, border_radius=border_radius)
    if shine:
        shine_rect = pygame.Rect(rect.x + 4, rect.y + 3, rect.width - 8, max(2, rect.height // 6))
...
def draw_corner_ornaments(surf, rect, color, size=22, thickness=3):
    """Dekorativní 'L' rohové zdobení (jako rámeček sběratelské karty) -
    kreslí se PO panelu, dává souhrnné kartě postavy honosnější vzhled."""
    rect = pygame.Rect(rect)
    corners = [
...
def build_background_surface():
    """Předpočítaný jemný svislý gradient na celou obrazovku (místo
    plochého jednobarevného pozadí) - počítá se jen jednou při startu,
    ne každý frame, takže na výkon nemá žádný vliv."""
    ...
    # Jemná, téměř neznatelná vinětace v rozích pro trochu hloubky.
